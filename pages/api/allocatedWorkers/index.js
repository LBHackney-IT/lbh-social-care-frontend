...
import {
  getAscAllocatedWorkers,
  addAscAllocatedWorker,
...
export default async (req, res) => {
  ...
  switch (req.method) {
    case 'GET':
      try {
        const data = await getAscAllocatedWorkers(req.query);
    ...
    case 'POST':
      try {
        const data = await addAscAllocatedWorker(req.query.id, req.body);
        res.status(HttpStatus.OK).json(data);
      } catch (error) {
